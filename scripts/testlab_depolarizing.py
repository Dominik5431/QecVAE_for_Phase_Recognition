...
from src.nn.data.depolarizing import DepolarizingSurfaceData
...
if task == 0:
    ...
    n1 = 10000
    # temperature = np.arange(0.8, 1.5, 0.05)
    temperature = np.arange(0.5, 3., 0.1)
    # temperature = np.array([])
    noises = np.exp(-4 / temperature) / (1/3 + np.exp(-4 / temperature))
    ...
    ax2.plot(4 / np.log(3 * (1 - noises) / noises), smooth(sus, 5), color='blue', label='susceptibility')
    ps = np.array(list(map(lambda x: np.exp(-4 / x) / (1/3 + np.exp(-4 / x)), temperature)))

    ax1.plot(temperature, 1 - 2 * 4 * ((1 - 2/3 * noises) * 2/3 * noises ** 3 + (1 - 2/3 * noises) ** 3 * 2/3 * noises), color='black',
             label='number of NTS')


    plt.vlines(1.565, 0, max(sus), colors='red', linestyles='dashed', label='threshold')
    # plt.vlines(1.465, 0, 0.5, colors='red', linestyles='dashed')
    # plt.vlines(1.250, 0, 0.5, colors='grey', linestyles='dashed')
