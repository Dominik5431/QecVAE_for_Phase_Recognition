...
def scatter_latent_var(latents: dict):
    ...
    for dist in dists:
        ...
        for j, noise in enumerate(noises):
            for k in range(len(latent[j][0])):
                plt.scatter(noise, latent[j][0][k].cpu())
        plt.show()
        for j, noise in enumerate(noises):
            for k in range(len(latent[j][1])):
                plt.scatter(noise, latent[j][1][k].cpu())
        plt.show()
...
def plot_accuracy(history, distance, val=True):
...
def plot_loss(history, distance, val=True):
    epochs = len(history.history['accuracy'])
...
def plot_prediction(dictionary, noises):
    """
    ...
    for i, dist in enumerate(dists):
        ...
        errors = errors / 1
        plt.errorbar(noises, predictions[:, 0], yerr=errors[:, 0], color=colors[i], marker=markers[i], linewidth=1,
                     label=dist)
        plt.errorbar(noises, predictions[:, 1], yerr=errors[:, 1], color=colors[i], marker=markers[i], linewidth=1,
                     label=dist)
    plt.legend()
