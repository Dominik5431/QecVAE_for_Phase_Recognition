...
class Functions:
    ...
    @classmethod
    def number_syndromes(cls, distance):
    ...
    @classmethod
    def sigmoid_der(cls, x, A, mu, sigma):
        return cls.sigmoid(x, A, mu, sigma) * (1 - cls.sigmoid(x, A, mu, sigma))

    @classmethod
    def sigmoid(cls, x, A, mu, sigma):
        return 1 / (1 + np.exp(-sigma * (x - mu)))

    @classmethod
    def linear(cls, x, m, n):
        return m * x + n
    ...
    @classmethod
    def data_collapse(cls, noises, dictionary):
        """
    ...
    @classmethod
    def find_crossing(cls, noises, predictions):
        """
    ...
    @classmethod
    def get_pcs(cls, dictionary, noises):
        """
        ...
        for i in np.arange(len(distances)):
            ...
            result[i, 1], result[i, 2] = cls.get_crossing_bootstrap(noises, predictions[i, :, 0],
                                                                    predictions_err[i, :, 0],
                                                                    noises,
    ...
    @classmethod
    def simple_bootstrap(cls, x, f=np.mean, c=0.68, r=100):
    ...
    @classmethod
    def get_crossing_mean(cls, x1, y1, err1, x2, y2, err2, xmin, xmax):
        spl1 = cls.get_spline(x1, y1, err1)
        spl2 = cls.get_spline(x2, y2, err2)
    ...
    @classmethod
    def get_crossing_bootstrap(cls, x1, y1, err1, x2, y2, err2, xmin, xmax):
        xcross = cls.get_crossing_mean(x1, y1, err1, x2, y2, err2, xmin, xmax)

        nsamples = 500

        samples = []
        for n in range(nsamples):
            x_sample = cls.get_crossing_mean(x1, np.random.normal(loc=y1, scale=err1), err1, x2,
